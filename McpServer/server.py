...
@mcp.tool
def document_search(query: str, folder_id: str = "global") -> dict:
    """
    ...
    try:
        ...
        query_params = {"embed_type": "query"}
        response = requests.post(
            WEAVIATE_SERVER, params=query_params, json={"text": [query]}
        )
        data = response.json()
...
def search_engine(query: str) -> SearchResponse:
    ...
    search_params = {"key": GOOGLE_SEARCH_KEY, "cx": CX, "q": query}
    try:
        ...
        return SearchResponse(
            results=[]
        )  # Make sure Agent workflow does not break if tool call fails.
...
if (
    __name__ == "__main__"
):  # For dev use case to run the file as script, so we define file specific entry point, can be run through CLI.
