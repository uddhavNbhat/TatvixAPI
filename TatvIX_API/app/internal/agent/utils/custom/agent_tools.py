...
class AgentTools:
    current_tools: ClassVar[List[str]] = ["document_search", "search_engine"]
    ...
    async def init_agent_tools(cls, tools: List[BaseTool]):
        ...
        self.tool_map = {
            "document_search": next(
                x.coroutine for x in tools if x.name == "document_search"
            ),
            "search_engine": next(
                x.coroutine for x in tools if x.name == "search_engine"
            ),
        }
        return self
