...
class Groq:
    ...
    def _init_model(self, model_name) -> ChatGroq:
        ...
        return ChatGroq(model=model_name, api_key=self._api_key, max_retries=2)
...
class Ollama:
    ...
    _supported_models: List[str] = ["qwen3:4b", "qwen3:8b"]
    ...
    def _sanitize_ssl_env() -> None:
        ...
        if ssl_cert_file and not Path(ssl_cert_file).exists():
            logger.warning(
                f"Invalid SSL_CERT_FILE detected: {ssl_cert_file}. Clearing it for Ollama client."
            )
        ...
        if ssl_cert_dir and not Path(ssl_cert_dir).exists():
            logger.warning(
                f"Invalid SSL_CERT_DIR detected: {ssl_cert_dir}. Clearing it for Ollama client."
            )
...
class ModelFactory:
    ...
    def initialize_model(
        ...
        return cls._providers[model_family](
            model_name
        )  # Return model instance accordingly.
