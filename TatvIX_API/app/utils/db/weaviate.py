...
async def get_data(session: SQLSessionDep, file_data: PopulateWeaviate) -> List[WeaviateMetaDataSchema]:
...
async def store_data(data: List[dict], weaviate_client: WeaviateClient):
    ...
    try:
        embeddings = weaviate_client.collections.get("Vectorbase")
        embedding_errors = 0
            ...
            with embeddings.batch.fixed_size(batch_size=200) as batch:
                for index, item in enumerate(data, start=1):
                    try:
                        ...
                        embedding_errors += 1
                        logger.error(f"Embedding/object preparation failed for row {index}: {item_error}")
                        if embedding_errors > 10:
                            logger.error("Batch import stopped due to excessive embedding/prep errors.")
                            break

                    if batch.number_errors > 10:
                        logger.error("Batch import stopped due to excessive Weaviate batch errors.")
                        break
        #Failed objects log
        ...
        if failed_objects:
            ...
            raise RuntimeError(f"Weaviate batch import failed for {len(failed_objects)} objects")
